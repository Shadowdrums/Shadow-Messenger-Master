...
class EncryptionHandler:
    ...
    def decrypt(self, encrypted_message: bytes) -> str:
        try:
            ...
            print(f"Decryption failed: {e} | Encrypted Message: {encrypted_message.hex()}")
            return ""
...
class ProtocolMessage:
    ...
    def from_bytes(data: bytes, encryption_handler: EncryptionHandler):
        ...
        try:
            ...
            print(f"Failed to parse message: {e} | Decrypted Message: {decrypted_message}")
            return None
...
class DiffieHellman:
    def __init__(self, key_length=2048):
        ...
        self.g = number.getPrime(self.key_length // 2)
...
def combine_key_halves(key_half1: bytes, key_half2: bytes, shared_secret: bytes) -> bytes:
    combined = key_half1 + key_half2
    print(f"Combining key halves: {key_half1.hex()} + {key_half2.hex()} with shared secret: {shared_secret.hex()}")
...
def insert_user(username, password, destination_ip, encryption_handler):
    ...
    encrypted_ip = encryption_handler.encrypt(destination_ip).hex() if encryption_handler else destination_ip
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
...
def diffie_hellman_key_exchange(conn: socket.socket, addr):
    try:
        ...
        full_key = combine_key_halves(key_half, other_key_half, str(shared_secret).encode())
